...
def Sphere2Cube(lon_deg, lat_deg, lat_dem_res, lon_dem_res, idFace, a=1):
    ...
    if idFace == 0:
        x = a * np.tan(lon)
        y = a * np.tan(lat) * (1.0 / np.cos(lon))
    elif idFace == 1:
        x = -a * 1.0 / np.tan(lon)
        y = a * np.tan(lat) * (1.0 / np.sin(lon))
    elif idFace == 2:
        x = a * np.tan(lon)
        y = -a * np.tan(lat) * (1.0 / np.cos(lon))
    elif idFace == 3:
        ...
        mask = np.abs(lat) == np.pi / 2.0
        x = a * np.sin(lon) * 1.0 / np.tan(lat)
        y = -a * np.cos(lon) * 1.0 / np.tan(lat)
        x[mask] = 0.0
        y[mask] = 0.0
    elif idFace == 5:
        mask = np.abs(lat) == np.pi / 2.0
...
def computeGrad_spherical(
    z_slice, lat_dem_res, lon_dem_res, LON, LAT, R=np.sqrt(3.0)
):
    ...
    d_z_lat = (1.0 / R) * d_z_lat_slice
    d_z_lon = 1.0 / (R * np.cos(np.radians(LAT))) * d_z_lon_slice
...
def localBasis2Spherical(x, y, lon_crop_extent, idFace, a=1):
    ...
    if idFace == 0 or idFace == 1 or idFace == 2 or idFace == 3:
        ...
        tx = -(
            (R * x * y)
            / (
                np.sqrt((R**2 * (a**2 + x**2)) / (a**2 + x**2 + y**2))
                * (a**2 + x**2 + y**2) ** (3.0 / 2.0)
            )
        )
        ty = np.sqrt(
            (R**2 * (a**2 + x**2)) / (a**2 + x**2 + y**2)
        ...
        tx = -(
            (a * R * x)
            / (
                np.sqrt((R**2 * (x**2 + y**2)) / (a**2 + x**2 + y**2))
                * (a**2 + x**2 + y**2) ** (3.0 / 2.0)
            )
        )
        ty = -(
...
def computeParDer(xx_1, xx_2, xx_3, z_node_triangle, iCubeFace):
    ...
    a2 = (x1 * z0 - x2 * z0 - x0 * z1 + x2 * z1 + x0 * z2 - x1 * z2) / (
        x1 * y0 - x2 * y0 - x0 * y1 + x2 * y1 + x0 * y2 - x1 * y2
    )
    a1 = (y2 * (-z0 + z1) + y1 * (z0 - z2) + y0 * (-z1 + z2)) / (
...
def compute_H1_seminorm_error(
    ...
    lat_interp_node, lon_interp_node = XYZ2LonLat(
        nodes[:, 0], nodes[:, 1], nodes[:, 2], lon_crop_extent
    )
    ...
    xyz_middle = (xyz[:, 0, :] + xyz[:, 1, :] + xyz[:, 2, :]) / 3.0
    lat_interp_middle, lon_interp_middle = XYZ2LonLat(
        xyz_middle[:, 0], xyz_middle[:, 1], xyz_middle[:, 2], lon_crop_extent
    )
    d_z_lat_middle = d_z_lat_interp(
        (lat_interp_middle, lon_interp_middle), method="nearest"
    )
    d_z_lon_middle = d_z_lon_interp(
        (lat_interp_middle, lon_interp_middle), method="nearest"
    )
    ...
    for iTri in range(xyz.shape[0]):
        ...
        if (
            np.allclose(xx_1[0], a)
            and np.allclose(xx_2[0], a)
            and np.allclose(xx_3[0], a)
        ):
            iCubeFace = 0
        elif (
            np.allclose(xx_1[1], a)
            and np.allclose(xx_2[1], a)
            and np.allclose(xx_3[1], a)
        ):
            iCubeFace = 1
        elif (
            np.allclose(xx_1[0], -a)
            ...
            np.allclose(xx_1[2], a)
            and np.allclose(xx_2[2], a)
            and np.allclose(xx_3[2], a)
        ):
            iCubeFace = 4
        elif (
            np.allclose(xx_1[2], -a)
        ...
        measure_tri = 0.5 * np.linalg.norm(np.cross(xx_2 - xx_1, xx_3 - xx_1))

        [Localx, Localy] = Global2Local(
            xyz_middle[0, 0], xyz_middle[0, 1], xyz_middle[0, 2], iCubeFace
        )
        [g_sqrt, ginv] = inverse_metric_tensor(Localx, Localy)

        [A, lx, ly, tx, ty] = localBasis2Spherical(
            Localx, Localy, lon_crop_extent, iCubeFace
        )
        ...
        vec_par_der_xy = computeParDer(
            xxl_1, xxl_2, xxl_3, z_node_triangle, iCubeFace
        )
        ...
        left_contr = (
            ...
            @ (
                np.array(
                    [
                        d_z_lon_middle[iTri] * lx + d_z_lat_middle[iTri] * tx,
                        d_z_lon_middle[iTri] * ly + d_z_lat_middle[iTri] * ty,
                    ]
                )
            )
            @ A
        ...
        eta_k += np.sqrt(
            np.linalg.norm(left_contr - right_contr) ** 2
            * measure_tri
            * g_sqrt
        )
...
def compute_size_field(
    ...
    for iTri in range(xyz.shape[0]):
        ...
        candidate = (
            tau * np.sqrt(measure_tri / N_elements / 0.5) / (eta_k + 1.0e-8)
        )

        for iPoi in range(3):
            sf[triangles[iTri][iPoi]] = np.minimum(
                sf[triangles[iTri][iPoi]],
                np.maximum(np.minimum(candidate, delta_max), delta_min),
            )

    for i in range(len(sf)):
        if sf[i] == 1e4:
            Poi = nodes[i]
            d = np.sum(np.abs(Poi - nodes) ** 2, axis=-1) ** (1.0 / 2.0)
...
delta_min = 1e-3
delta_max = 0.01  # coarsest mesh resolution
###################################
...
lon_id = [
    GetId(lon_dem[0], lon_crop_extent[0], lon_dem_res),
    GetId(lon_dem[0], lon_crop_extent[1], lon_dem_res),
]
lat_id = [
    GetId(lat_dem[0], lat_crop_extent[0], lat_dem_res),
    GetId(lat_dem[0], lat_crop_extent[1], lat_dem_res),
]
...
z_slice = z_dem[lat_id[0] : lat_id[1], range(lon_id[0], lon_id[1])]
print("Data loaded")
DEM.close()
...
z_slice = (
    z_slice.data / earth_radius * np.sqrt(3)
)  # get adimensional topography

if (
    lon_slice[0] > lon_slice[-1]
):  # non ascending order, pb with RegularGridInterpolator
    lon_slice[lon_slice >= lon_slice[0]] -= 360.0
...
z_mesh_fem = interpolate.RegularGridInterpolator(
    (lat_slice, lon_slice),
    z_slice,
    fill_value=None,
    bounds_error=False,
...
d_z_lat, d_z_lon = computeGrad_spherical(
    z_slice, lat_dem_res, lon_dem_res, LON, LAT
)
d_z_lat_fem = interpolate.RegularGridInterpolator(
    (lat_slice, lon_slice),
    d_z_lat,
    fill_value=None,
    bounds_error=False,
...
d_z_lon_fem = interpolate.RegularGridInterpolator(
    (lat_slice, lon_slice),
    d_z_lon,
    fill_value=None,
    bounds_error=False,
...
plt.contourf(
    LON, LAT, np.arctan(np.sqrt(d_z_lat**2 + d_z_lon**2)) * 180 / np.pi
)
plt.axis("equal")
plt.xlabel("longitude (deg)")
plt.ylabel("latitude (deg)")
...
candidate = (
    np.arccos(np.tan(lat_crop_extent[0] * np.pi / 180.0)) * 180.0 / np.pi
)
...
candidate = (
    np.arccos(np.tan(lat_crop_extent[-1] * np.pi / 180.0)) * 180.0 / np.pi
)
if candidate > lon_crop_extent[0] and candidate < lon_crop_extent[-1]:
    lon_slice_mod = np.append(lon_slice_mod, candidate)
...
candidate = (
    -np.arcsin(np.tan(lat_crop_extent[0] * np.pi / 180.0)) * 180.0 / np.pi
)
if candidate > lon_crop_extent[0] and candidate < lon_crop_extent[-1]:
    lon_slice_mod = np.append(lon_slice_mod, candidate)
...
candidate = (
    -np.arcsin(np.tan(lat_crop_extent[-1] * np.pi / 180.0)) * 180.0 / np.pi
)
if candidate > lon_crop_extent[0] and candidate < lon_crop_extent[-1]:
    lon_slice_mod = np.append(lon_slice_mod, candidate)
...
lat_app_candidate = (
    np.arctan(-np.cos(lon_slice_mod * np.pi / 180.0)) * 180.0 / np.pi
)
lat_app_candidate = lat_app_candidate[
    np.logical_and(
        lat_app_candidate >= lat_crop_extent[0],
        lat_app_candidate <= lat_crop_extent[-1],
    )
]
...
lat_app_candidate = (
    np.arctan(-np.sin(lon_slice_mod * np.pi / 180.0)) * 180.0 / np.pi
)
lat_app_candidate = lat_app_candidate[
    np.logical_and(
        lat_app_candidate >= lat_crop_extent[0],
...
mask_crop_boundary = mask_crop_boundary.astype(int) - ndimage.binary_erosion(
    mask_crop_boundary.astype(int)
)
...
if mask_crop_boundary_mod[LON_mod == 45].size > 0:
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == 45, LAT_mod == lat_slice_mod[0])
    ] += 1
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == 45, LAT_mod == lat_slice_mod[-1])
    ] += 1
...
if mask_crop_boundary_mod[LON_mod == 135].size > 0:
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == 135, LAT_mod == lat_slice_mod[0])
    ] += 1
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == 135, LAT_mod == lat_slice_mod[-1])
    ] += 1
...
if mask_crop_boundary_mod[LON_mod == -225].size > 0:
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == -225, LAT_mod == lat_slice_mod[0])
    ] += 1
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == -225, LAT_mod == lat_slice_mod[-1])
    ] += 1

if mask_crop_boundary_mod[LON_mod == -315].size > 0:
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == -315, LAT_mod == lat_slice_mod[0])
    ] += 1
    mask_crop_boundary_mod[
        np.logical_and(LON_mod == -315, LAT_mod == lat_slice_mod[-1])
    ] += 1
...
mask_crop_boundary_mod[
    np.abs(
        LAT_mod - np.arctan(-np.cos(LON_mod * np.pi / 180.0)) * 180.0 / np.pi
    )
    < toll
] += 1
mask_crop_boundary_mod[
...
mask_crop_boundary_mod[
    np.abs(
        LAT_mod - np.arctan(-np.sin(LON_mod * np.pi / 180.0)) * 180.0 / np.pi
    )
    < toll
] += 1
mask_crop_boundary_mod[
...
plt.plot(
    LON_mod[mask_crop_boundary_mod > 0],
    LAT_mod[mask_crop_boundary_mod > 0],
    "o",
)
plt.axis("equal")
plt.xlabel("longitude (deg)")
...
mask = np.logical_and(
    np.abs(LAT) < np.arctan(np.cos(LON * np.pi / 180.0)) * 180.0 / np.pi,
    np.cos(LON * np.pi / 180) > 1.0 / np.sqrt(2.0),
)
if LAT[mask].size != 0 and LON[mask].size != 0:
    ...
    bool_val = np.logical_not(
        np.logical_and(
            np.abs(LAT_mod)
            < np.arctan(np.cos(LON_mod * np.pi / 180.0)) * 180.0 / np.pi
            + toll,
            np.cos(LON_mod * np.pi / 180) > 1.0 / np.sqrt(2.0) - toll,
        )
    )
    ...
    for ii in range(LON_mod[mask_crop_boundary_p].size):
        p1 = [
            LON_mod[mask_crop_boundary_p][ii],
            LAT_mod[mask_crop_boundary_p][ii],
        ]
        points.append(p1)
...
mask = np.logical_and(
    np.abs(LAT) < np.arctan(np.sin(LON * np.pi / 180.0)) * 180.0 / np.pi,
    np.sin(LON * np.pi / 180) > 1.0 / np.sqrt(2.0),
)
if LAT[mask].size != 0 and LON[mask].size != 0:
    isFaceP2 = True
    ...
    bool_val = np.logical_not(
        np.logical_and(
            np.abs(LAT_mod)
            < np.arctan(np.sin(LON_mod * np.pi / 180.0)) * 180.0 / np.pi
            + toll,
            np.sin(LON_mod * np.pi / 180) > 1.0 / np.sqrt(2.0) - toll,
        )
    )
    mask_crop_boundary_p = mask_crop_boundary_mod.astype(bool)
    mask_crop_boundary_p[bool_val] = False
...
mask = np.logical_and(
    LAT > np.arctan(np.cos(LON * np.pi / 180.0)) * 180.0 / np.pi,
    np.cos(LON * np.pi / 180) > 1.0 / np.sqrt(2.0),
)
mask = np.logical_or(
    mask,
    np.logical_and(
        LAT > np.arctan(np.sin(LON * np.pi / 180.0)) * 180.0 / np.pi,
        np.sin(LON * np.pi / 180) > 1.0 / np.sqrt(2.0),
    ),
)
mask = np.logical_or(
...
if LAT[mask].size != 0 and LON[mask].size != 0:
    ...
    bool_val = np.logical_and(
        LAT_mod
        > np.arctan(np.cos(LON_mod * np.pi / 180.0)) * 180.0 / np.pi - toll,
        np.cos(LON_mod * np.pi / 180) > 1.0 / np.sqrt(2.0) - toll,
    )
    bool_val = np.logical_or(
        bool_val,
        np.logical_and(
            LAT_mod
            > np.arctan(np.sin(LON_mod * np.pi / 180.0)) * 180.0 / np.pi
        ...
        for jj in range(tri[ii].simplices.shape[0]):
            ...
            for iVert in range(3):
                ...
                if bool_val1 and bool_val2:
                    ...
                    XX, YY = Sphere2Cube(
                        p1_vect, p2_vect, lat_dem_res, lon_dem_res, ii
                    )
                    ...
                    for kk in range(np.size(X_gl)):
                        candidate = [
                            np.allclose([X_gl[kk], Y_gl[kk], Z_gl[kk]], point)
                            for point in pointPhy_globalList
                        ]

                        if (
                            not (np.sum(candidate) > 0)
                            or np.size(candidate) == 0
                        ):
                            gmsh.model.geo.addPoint(
                                X_gl[kk],
                                Y_gl[kk],
                                Z_gl[kk],
                                meshSize=starting_mesh_size,
                                tag=index,
                            )
                            pointPhy_globalList.append(
                                [X_gl[kk], Y_gl[kk], Z_gl[kk]]
                            )
                            pList.append(index)
                            index += 1
                        else:
...
# boundary nodes
nodes_coords = np.reshape(
    gmsh.model.mesh.getNodesForPhysicalGroup(1, 1)[1], (-1, 3)
)
nodes_ids = gmsh.model.mesh.getNodesForPhysicalGroup(1, 1)[0]


[sf_ele, act_err] = compute_size_field(
    mesh.vxyz,
    mesh.triangles,
    z_mesh_fem,
    d_z_lat_fem,
...
gmsh.view.addModelData(
    sf_view, 0, current_model, "NodeData", tags_tot, sf_tot[:, None]
)
...
act_err = compute_H1_seminorm_error(
    mesh.vxyz,
    mesh.triangles,
    z_mesh_fem,
    d_z_lat_fem,
